...
if ($_FILES["file"]["error"] > 0)
  ...
  $highestColumn = $sheet->getHighestColumn(); //取得总列数
	...
	for($j=3;$j<=$highestRow;$j++)
	{
		...
		$i = getExcelVal($objPHPExcel->getActiveSheet()->getCell("I".$j));//获取B列的值
		echo $a.'==='.$b;
				echo '<br>';
		$sql = "INSERT INTO ruku (xuhao,riqi,jitai,pihao,guige,dengji,jianshu,zhongliang,beizhu) VALUES('".$a."','".$b."','".$c."','".$d."','".$e."','".$f."','".$g."','".$h."','".$i."')";
		echo $sql;
		echo '<br>';
		mysql_query($sql);
		$month = substr($b,0,7).'-01';
		echo $month;
		updateKucun($month,$d,$e,$g,$h);
		updateKucunTongji($month,$d,$e,$g,$h);
    ...
    while ($row=mysql_fetch_array($qres))
    {
		...
		if($cnt>0){
			...
			$isnertSql = "insert into kucun (pihao,guige,month,jianshu,zhongliang) values ('".$pihao."','".$guige."','".$month."','".$jianshu."','".$zhongliang."')";
    ...
    while ($row=mysql_fetch_array($qres))
    {
		...
		if($cnt>0){
			$updateSql = "update kucuntongji set rukujianshu = rukujianshu + ".$jianshu.",rukuzhongliang = rukuzhongliang+ '".$zhongliang."' where pihao = '".$pihao."' and month='".$month."' and guige = '".$guige."'";
			echo "==updateSql".$updateSql."==";
			mysql_query($updateSql);
		}else{
			$isnertSql = "insert into kucuntongji (pihao,guige,month,rukujianshu,rukuzhongliang,chukujianshu,chukuzhongliang) values ('".$pihao."','".$guige."','".$month."','".$jianshu."','".$zhongliang."',0,0)";
			echo "==isnertSql".$isnertSql."==";
			mysql_query($isnertSql);
		}
	}
