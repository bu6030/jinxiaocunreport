...
客户名称：<input type="text" name="kehumingcheng" id="kehumingcheng"/> 
...
<?php
    ...
    echo "<tr><td colspan='14' align='center'>能德化纤产品（DTY）出库明细</td></tr>";
    echo "<tr align='center'><td>序号</td><td>日期</td><td>机台</td><td>名称</td><td>规格</td><td>等级</td><td>单位</td><td>数量</td><td>定重</td><td>重量</td><td>单价</td><td>金额</td><td>客户名称</td><td>修改</td></tr>";
		...
		$sql = "select * from chuku where riqi <='".$dateTo."' and riqi >='".$dateFrom."'";
		if(isset($_POST['kehumingcheng'])){
			$kehumingcheng = $_POST['kehumingcheng'];
			$sql = $sql." and kehumingcheng like '%".$kehumingcheng."%' ";
		}
	    	...
	    	while ($row=mysql_fetch_array($qres))
			  {
			    ...
			    echo "<td>".$row['riqi']."</td>";
					...
					echo "<td>".$row['danjia']."</td>";
					echo "<td>".$row['jine']."</td>";
					echo "<td>".$row['kehumingcheng']."</td>";
					echo "<td><a href='updateChuku.php?id=".$row['id']."'>修改</href></td></tr>";
					echo "</tr>";
					$num = $num+1;
			  }
	  }
