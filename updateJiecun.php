...
include("conn.php"); 
$id = $_GET['id'];
$sql = "select * from jiecun where id = ".$id;
$qres=mysql_query($sql);
if($qres){
	while ($row=mysql_fetch_array($qres)){
		$riqi = $row['riqi'];
		$riqi = substr($riqi,0,7);
		$jitai = $row['jitai'];
...
结存修改
<form action="updateJiecunSubmit.php" method="post" enctype="multipart/form-data">
	<input type="hidden" name="id" id="id" value ="<?php echo $id ?>" />
日期：<input type="text" name="riqi" id="riqi" placeholder="2017-08-01" value ="<?php echo $riqi ?>"/> 例如要录入2017-08月初的库存，则需要输入2017-08<br/>
